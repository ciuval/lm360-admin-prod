...
export default function HeaderBar({ unread = 0, matches = 0, visite = 0 }) {
      ...
      <header style={wrap} role="banner" aria-label="Barra di navigazione">
        <div style={inner}>
          ...
          <nav aria-label="Navigazione principale">
            <ul style={navList}>
              <NavItem to="/guide" current={path.startsWith('/guide')}>
                Guide
              </NavItem>

              {/* Rotte esistenti: NON rimuovere */}
              <NavItem to="/profili-pubblici" current={path.startsWith('/profili')}>
                Profili
              </NavItem>
              ...
              <NavItem to="/premium" current={path.startsWith('/premium')}>
                Premium
              </NavItem>
            </ul>
          </nav>
...
function badge(n, label) {
  ...
  return (
    <span style={pill} aria-label={`${txt} ${label}`} title={`${txt} ${label}`}>
      {txt}
    </span>
  );
}
