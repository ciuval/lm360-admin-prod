...
export default function ChatIndex({ userId }) {
  ...
  useEffect(() => {
    ...
    const peers = Array.from(
      new Set(rows.map((r) => (r.mittente_id === userId ? r.destinatario_id : r.mittente_id)))
    ).filter(Boolean);
    if (!peers.length) return;

    (async () => {
      try {
        const { data } = await supabase.from('profili').select('*').in('id', peers);
        const map = {};
        (data || []).forEach((p) => {
          map[p.id] = p;
        });
  ...
  const conv = useMemo(() => {
    ...
    for (const m of rows) {
      ...
      if (!entry.last) entry.last = m; // lista giÃ  desc â†’ primo visto Ã¨ lâ€™ultimo
      entry.total++;
    ...
    return Array.from(byPeer.entries()).map(([peerId, info]) => {
      ...
      const title = p.nome || p.username || p.email || peerId?.slice(0, 6) + 'â€¦';
      const avatar = p.avatar_url || p.photo_url || null;

      // trova un testo anteprima tra varie possibili chiavi
      const last = info.last || {};
      const preview =
        last.testo ?? last.text ?? last.message ?? last.body ?? last.contenuto ?? 'â€”';
  ...
  return (
    <section className="lm360-section">
      ...
      {loading ? (
        ...
        <div className="muted" style={{ padding: 12 }}>
          Nessuna chat ancora. Inizia dai <a href="#/match">Match</a> o esplora{' '}
          <a href="#/profili-pubblici">Persone</a>.
        </div>
      ) : (
