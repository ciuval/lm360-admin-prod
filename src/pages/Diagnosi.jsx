...
function suggestTrack(ans) {
  let base = 'starter';
  const area = ans.q1 === 'relazioni' ? 'rel' : ans.q1 === 'performance' ? 'biz' : 'focus';
  const level = ans.q2 === 'molto' ? 'pro' : ans.q2 === 'medio' ? 'light' : 'quick';
...
export default function Diagnosi() {
  ...
  const STORAGE_KEY = 'diagnosi-answers';
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (uid) {
        ...
        if (error) {
          // no PII
          console.warn('[Diagnosi] insert skip:', error.message);
        }
      }
