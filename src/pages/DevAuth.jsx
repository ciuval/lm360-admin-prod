...
export default function DevAuth() {
  ...
  const signupPwd = async () => {
    setErr('');
    setMsg('signup… (può richiedere conferma email se attiva)');
    const { data, error } = await supabase.auth.signUp({ email, password });
    if (error) setErr(error.message);
    else setMsg('signup ok (controlla Users o email)');
    await refresh();
  };
      ...
      <div className="mt-4 p-4 rounded-md border border-zinc-700 bg-zinc-900">
        ...
        <div className="grid gap-2 max-w-sm">
          ...
          <div className="flex gap-2 mt-2">
            <button onClick={loginPwd} className="px-3 py-1 rounded bg-indigo-600">
              Login (pwd)
            </button>
            <button onClick={signupPwd} className="px-3 py-1 rounded bg-zinc-700">
              Signup (pwd)
            </button>
            <button onClick={logout} className="px-3 py-1 rounded bg-zinc-700">
              Logout
            </button>
            <button onClick={refresh} className="px-3 py-1 rounded bg-zinc-700">
              Refresh
            </button>
          </div>
        </div>
