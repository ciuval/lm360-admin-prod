...
export default function LoginWithMFA() {
  ...
  const handleLogin = async () => {
    ...
    if (error?.message?.includes('MFA required')) {
      ...
      setMfaRequired(true);
      toast('ðŸ” MFA richiesta. Inserisci il codice a 6 cifre.');
    } else if (error) {
      toast.error('Login fallito');
    } else {
      toast.success('âœ… Login riuscito senza MFA');
  ...
  const handleVerifyMFA = async () => {
    ...
    if (error) return toast.error('Codice MFA errato âŒ');
    toast.success('âœ… Autenticazione completata!');
    window.location.href = '/profilo';
  };
  ...
  return (
    <div style={containerStyle}>
      ...
      <h2 style={titleStyle}>ðŸ” Login protetto</h2>
      ...
      {!mfaRequired ? (
        ...
        <>
          <input
            ...
            style={inputStyle}
          />
          <button onClick={handleVerifyMFA} style={btnStyle}>
            âœ… Verifica MFA
          </button>
        </>
      )}
