  ...
  async function load() {
    ...
    try {
      ...
      const all = [
        {
          id: 'x1',
          titolo: 'Benvenuto su LM360',
          slug: 'benvenuto-lm360',
          sommario: 'Da dove iniziare per ottenere valore in 24 ore.',
          cover_url: 'https://placehold.co/600x400?text=Benvenuto',
          premium_only: false,
        ...
        },
        {
          ...
          slug: 'diagnosi-rapida',
          sommario: 'Questionario in 60s per capire il percorso.',
          cover_url: 'https://placehold.co/600x400?text=Diagnosi',
          premium_only: false,
          created_at: new Date().toISOString(),
        },
        {
          ...
          titolo: 'Percorsi PRO',
          slug: 'percorsi-pro',
          sommario: 'Template e modelli per accelerare.',
          cover_url: 'https://placehold.co/600x400?text=PRO',
          premium_only: true,
          created_at: new Date().toISOString(),
        },
      ];
    ...
    <div style={{ maxWidth: 980, margin: '0 auto', padding: '1rem' }}>
      ...
      >
        <div role="group" aria-label="Filtra guide" style={{ display: 'flex', gap: 8 }}>
          ...
          <button
            ...
            style={segBtn(filter === 'free')}
          >
            Solo Free
          </button>
          <button
            onClick={() => setFilter('premium')}
            aria-pressed={filter === 'premium'}
            style={segBtn(filter === 'premium')}
          >
            Solo Premium
          </button>
        </div>
        ...
        <Link to="/diagnosi" style={btnPrimary}>
          🔎 Diagnosi (60s)
        </Link>
      ...
      {!loading && filtered.length === 0 && (
        <div style={{ opacity: 0.8, marginTop: 16 }}>
          {filter === 'premium' ? (
            <p>
              Nessuna guida Premium visibile.{' '}
              {isAuth ? (
                <>
                  Se non hai Premium,{' '}
                  <Link style={link} to="/premium">
                    sblocca l’accesso
                  </Link>
                  .
                </>
                ...
                <>
                  Accedi per vedere se hai accesso, oppure{' '}
                  <Link style={link} to="/premium">
                    attiva Premium
                  </Link>
                  .
