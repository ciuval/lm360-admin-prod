...
export default function DebugPage() {
  ...
  const testLogin = async () => {
    ...
    if (error) return setOutput(`âŒ Errore login: ${error.message}`);
    setUser(data.user);
  ...
  const testProfilo = async () => {
    ...
    if (!user) return setOutput('âŒ Nessun utente loggato');
    setUser(user);
    fetchDati(user);
  };
  ...
  const handleLogout = async () => {
    ...
    if (error) return setOutput(`âŒ Logout fallito: ${error.message}`);
    setUser(null);
    setProfilo(null);
    setMatch([]);
    setMessaggi([]);
  ...
  return (
    <div style={container}>
      <h2 style={title}>ðŸ§ª Debug Page LoveMatch360</h2>
      ...
      <div style={buttonGroup}>
        <button onClick={testLogin} style={buttonGreen}>
          ðŸ” Login
        </button>
        <button onClick={testProfilo} style={buttonBlue}>
          ðŸ“˜ Test Profilo
        </button>
      ...
      {user && (
        <div style={dataBox}>
          <h3>ðŸ” Utente Loggato</h3>
          <pre>{JSON.stringify(user, null, 2)}</pre>

          <h3>ðŸ™‹ Profilo</h3>
          <pre>{JSON.stringify(profilo, null, 2)}</pre>

          <h3>ðŸ’˜ Match Attivi</h3>
          <pre>{JSON.stringify(match, null, 2)}</pre>

          <h3>ðŸ’¬ Ultimi Messaggi</h3>
