...
export default function AbTestDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!mounted) return;
      if (error) {
        console.error('ab_events load:', error.message);
        setRows([]);
      } else setRows(data || []);
      setLoading(false);
    })();
  ...
  const agg = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      if (!map.has(key))
        map.set(key, { test: r.test, variant: r.variant, exposure: 0, click: 0, conversion: 0 });
      const o = map.get(key);
  ...
  const Card = ({ title, children }) => (
    <div className="bg-[#1e1e1e] rounded-xl p-6 shadow border border-zinc-800">
      {children || <div />}
      ...
      {loading ? (
        ...
        <>
          <div className="grid md:grid-cols-3 gap-4 mb-6">
            ...
            <Card title="Eventi grezzi">
              <div className="text-gray-300 text-sm">
                exposure: {rows.filter((r) => r.event === 'exposure').length} Â· click:{' '}
                {rows.filter((r) => r.event.startsWith('click')).length} Â· conversion:{' '}
                {rows.filter((r) => r.event === 'conversion').length}
              </div>
            </Card>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <ChartBars
              title="Hero CTR"
              data={agg.hero.map((i) => ({ name: i.variant, value: i.ctr }))}
            />
            <ChartBars
              title="Paywall Conversion"
              data={agg.paywall.map((i) => ({ name: i.variant, value: i.conv }))}
            />
          </div>
        </>
      )}
