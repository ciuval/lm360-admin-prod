...
Write-Host "== LM360 Router Patch: start ==" -ForegroundColor Cyan

# Trova main.jsx/tsx
$main = Get-ChildItem -Path . -Recurse -Include main.jsx,main.tsx | Select-Object -First 1
if (-not $main) { Write-Error "main.jsx/tsx non trovato. Dimmi il path esatto e lo adatto."; exit 1 }
$base = $main.DirectoryName
$ext  = (if ($main.Name -like "*.tsx") { "tsx" } else { "jsx" })
Write-Host ("main: " + $main.FullName) -ForegroundColor Green

# Installa react-router-dom se manca (safe)
try {
  ...
  if (-not $hasRRD) { npm i react-router-dom@^6 }
} catch {}

# Helper: scrivi solo se mancante
function Write-IfMissing([string]$path,[string]$content){
  if (-not (Test-Path $path)) { New-Item -Force -ItemType Directory (Split-Path $path -Parent) | Out-Null; Set-Content -Encoding UTF8 -Path $path -Value $content; Write-Host "Creato: $path" }
...
function PageStub([string]$name,[string]$desc) {
  return @"
import React from "react";
...
# Sostituzioni comuni: <App /> -> <AppRouter />
...
Write-Host "== LM360 Router Patch: done ==" -ForegroundColor Cyan
