      ...
      <ul role="list" style={grid}>
        {filtered.map((p) => (
          <li key={p.id} style={card}>
            ...
            <Link to={`/profilo/${p.id}`} style={linkBtn} aria-label={`Vedi profilo di ${p.nome || "utente"}`}>
              Vedi profilo
            </Link>
          </li>
        ))}
...
const grid = { display: "grid", gridTemplateColumns: "1fr", gap: 10, listStyle: "none", padding: 0, margin: 0 };
const card = { display: "flex", alignItems: "center", gap: 12, padding: 12, border: "1px solid #2b2b2b", borderRadius: 10, background: "#181818" };
const avatar = { borderRadius: "50%", objectFit: "cover", background: "#2b2b2b" };
const nameRow = { display: "flex", alignItems: "center", gap: 8 };
const roleBadge = { fontSize: 12, border: "1px solid #444", borderRadius: 999, padding: "2px 8px", opacity: 0.8 };
