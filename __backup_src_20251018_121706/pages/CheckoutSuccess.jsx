...
export default function CheckoutSuccess(){
  ...
  useEffect(()=>{ try{ ldOn(); }catch{} const t=setTimeout(()=>{ try{ldOff();}catch{} }, 600); return ()=>clearTimeout(t); },[]);
  ...
  return (
    <section className="p-8 bg-dark text-light min-h-screen flex items-center justify-center">
      <div className="bg-[#1e1e1e] rounded-xl shadow-lg p-8 max-w-lg text-center">
        ...
        <p className="text-gray-300 mb-6">Hai sbloccato filtri pro, chat prioritaria e libreria pratica.</p>
        <button className="px-6 py-3 rounded-xl bg-primary text-white font-semibold hover:bg-pink-600 transition" onClick={() => nav("/profilo", { replace: true })}>
          Vai al Profilo
        </button>
        <div className="mt-4 text-sm text-gray-400">
          Vuoi vedere subito i tuoi <strong>Match</strong>?{" "}
          <button className="text-primary hover:underline" onClick={() => nav("/match")}>Vai ai Match 💘</button>
        </div>
        <div className="mt-4 text-xs text-gray-500">Gestisci l’abbonamento dal tuo profilo (Billing Portal).</div>
      </div>
    </section>
  );
}
