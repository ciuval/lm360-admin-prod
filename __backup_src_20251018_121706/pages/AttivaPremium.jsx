...
// LoveMatch360 — Checkout reale (subscription) + Customer Portal
...
export default function AttivaPremium() {
  ...
  useEffect(() => {
    ...
    if (q.get("canceled") === "1") {
      toast("Checkout annullato. Nessun addebito ❤️");
    }
  }, [location.search]);
  ...
  async function openPortal() {
    if (!user) {
      toast.error("Accedi per gestire l’abbonamento.");
      navigate("/login");
      return;
    }
    try {
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "1.25rem", color: "#fff" }}>
      <h1 style={{ fontSize: 28, marginBottom: 8 }}>LoveMatch360 Premium 💎</h1>
      <p style={{ opacity: 0.9, marginBottom: 16 }}>
        Match intelligenti, ambiente sicuro, vibe elegante. Nessun caos.
      </p>

      {/* Scelta piano */}
      <div role="radiogroup" aria-label="Seleziona il piano" style={{
        ...
        <label style={labelStyle(plan === "premium_yearly")}>
          ...
          />
          <span>Annuale</span>
          <span style={pill}>Risparmi di più</span>
        </label>
      </div>
      ...
      <button
        ...
        style={ctaPrimary}
      >
        {loading ? "Reindirizzamento…" : "✨ Attiva Premium"}
      </button>
      ...
      <div style={{ marginTop: 16 }}>
        <button
          ...
          style={ctaSecondary}
        >
          {loadingPortal ? "Apertura…" : "Gestisci abbonamento (Portal)"}
        </button>
      </div>
