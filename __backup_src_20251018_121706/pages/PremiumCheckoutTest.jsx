...
export default function PremiumCheckoutTest() {
  ...
  const [tok, setTok] = useState(
    localStorage.getItem('LM_TOK') || localStorage.getItem('lm_tok') || ''
  );
      ...
      try {
        if (!tok) {
          ...
          if (rTok.ok) {
            ...
            localStorage.setItem('LM_TOK', j.access_token);
          }
        }
      } catch {}
      try {
      ...
      <ul style={{ listStyle:'none', padding:0 }}>
        {prices.map(p => (
          <li key={p.price_id} style={{ margin:'6px 0' }}>
            <label>
              <input type="radio" name="p" onChange={() => setChosen(p.price_id)} />
              {' '}
