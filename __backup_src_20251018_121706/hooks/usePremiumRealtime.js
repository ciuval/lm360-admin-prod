  ...
  React.useEffect(() => {
    ...
    async function fetchPremium(){
      try {
        ...
        const r = await fetch("/api/my-premium", { headers:{ Authorization:`Bearer ${tok?.access_token||""}` } });
        const j = await r.json();
        return !!j?.premium;
      } catch { return null; }
    }
    ...
    function startPolling(){
      ...
      pollRef.current = setInterval(async () => {
        ...
        if (curr == null) return;
        if (lastActiveRef.current === null) { lastActiveRef.current = curr; return; }
    ...
    async function startRealtime(){
      ...
      if (!user_id || !supabase?.channel) { startPolling(); return; }
      ...
      try {
        ...
        ch.on("postgres_changes", {
          ...
          const active = row?.status === "active" && row?.scadenza && (new Date(row.scadenza) > new Date());
        ...
        setTimeout(() => { if (!subscribed) startPolling(); }, 1600);

        subCleanupRef.current = () => { try { supabase.removeChannel(ch); } catch {} };
      } catch {
        startPolling();
      }
    }
